    ...
    configureServer(server) {
      server.middlewares.use('/generate/subtitles', async (req, res) => {
        if (req.method === 'GET') {
          ...
          try {
            const bytes = await readFile(resolve(UPLOAD_DIR, match[1]));
        ...
        try {
          ...
          await writeFile(resolve(UPLOAD_DIR, filename), serialize(input.cues, input.format), 'utf8');
