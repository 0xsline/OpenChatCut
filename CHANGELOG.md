...
- Added opt-in blurred background fill for video and image clips: the Inspector and `edit_item` can enable a synchronized blurred/darkened cover layer, choose soft/medium/strong/maximum blur presets, and edit the sharp foreground through an independent move/resize/crop/rotate frame. Shared preview/export compositing preserves fades, effects, and GLSL transition alpha, while FCPXML export reports the unsupported layer explicitly.
  新增视频与图片片段的可选模糊背景填充：可通过检查器或 `edit_item` 启用同步的模糊压暗铺满层，选择轻度、标准、强烈、极强四档效果，并通过独立的清晰前景框自由移动、缩放、裁剪和旋转。预览与导出共用合成链并保留淡化、特效和 GLSL 转场透明度，FCPXML 导出会显式报告该格式不支持的图层。
