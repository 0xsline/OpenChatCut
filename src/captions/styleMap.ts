...
// Map a source edit_captions `style` json payload onto the clone's caption style
// surface. The source exposes a rich style vocabulary; this build's captions are
...
/** source style fields with no representable target in this build → reported, not applied. */
...
/** source pacing vocab → clone's two-mode pacing (auto/phrase/sentence → phrase). */
...
export function mapCaptionStyle(json: Record<string, unknown>, canvasHeight: number): StyleMapResult {
  ...
  // report anything the source sent that this build can't represent
