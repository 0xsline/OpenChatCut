// 内容感知字幕分段——源站词表/权重(逐字照搬,不许改动)。
// 全部常量挖自反编译 bundle:
//   ~/Desktop/project/chatcut-reverse/resources/bundles/decompiled/editor/entry.js
// 每组注释标注 bundle 内的定位正则(python3 re 搜索用),便于日后复核。

/** 英文断点模式表(bundle 定位 /z9e\s*=/):左词命中 → 该词后是好断点。 */
export const LATIN_BREAK_PATTERNS: ReadonlyArray<{ pattern: RegExp; score: number }> = [
  { pattern: /[.!?:;,]$/, score: 100 },
];

/** 英文避断惩罚对(bundle 定位 /U9e\s*=/,12 条,顺序敏感——首个命中即停)。
...
/** 英文短功能词(bundle 定位 /j9e\s*=/):孤词风险——H9e 里当下一词命中且剩词 ≤2 时 −40。 */
...
/** 语气词(bundle 定位 /const W9e = \[/):位于左词末尾且右词以 CJK 开头 → 好断点(其后断,优先级 60)。 */
export const MODAL_PARTICLES = ['啊', '吧', '呗', '哈', '啦', '嘛', '呢', '哦', '呀'] as const;

/** 结构助词/粘着词表(bundle 定位 /const Q9 = \[/,含日/韩助词):
 * mA() 孤词避断——左词末字或右词首字命中 → 该断点标记 orphanRisk(选断点时降权 30)。 */
...
/** 不可作行首的助词(bundle 定位 /const G9e = \[/):FHe 后处理——若某页以这些字开头,
 * 从上一页拉词并入本页。 */
...
/** 英文功能词(bundle 定位 /const Bne = \[/):cP() 判 latinFunction 孤词风险用。 */
...
/** 量词表(bundle 定位 /function w5\(/ 内联数组):「X of」不拆(latinQuantifierOf 孤词风险)。 */
...
/** 停顿降噪连接词(bundle 定位 /function gHe\(/ 内联数组):这些词带逗号结尾时,
...
/** CJK 词缀(bundle 定位 /const y1e = \[/):hI() 分词兜底——左字为 CJK 且右字命中 → 不可拆。 */
export const CJK_WORD_SUFFIXES = ['们', '化', '性', '者', '度', '流', '栈', '后'] as const;

/** CJK 疑问句式(bundle 定位 /function Hne\(/、/function lHe\(/ 的两条正则,优先级 58 断点)。 */
...
/** 停顿断点优先级(bundle 定位 /const V9e = \(t\)=>/):间隔 ms → priority。 */
...
/** 选断点时孤词风险降权(bundle 定位 /function uHe\(/ 内 `const s = 30`)。 */
export const ORPHAN_PICK_DEMOTION = 30;

/** 停顿成为断点的最小间隔与降噪阈值(bundle 定位 /d >= 150 && \(!gHe/)。 */
