...
// User-saved caption style presets (source edit_captions preset_save/apply/list/rename/
...
const DB_NAME = 'chatcut-captions';
const STORE = 'presets';
...
export async function listCaptionPresets(): Promise<CaptionPreset[]> {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.onsuccess = () => resolve((req.result as CaptionPreset[]).sort((a, b) => a.createdAt - b.createdAt));
    req.onerror = () => reject(req.error);
  });
}
