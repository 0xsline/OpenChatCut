// 内容感知字幕分段引擎——按源站语义断行/分页(词表/权重见 segmenterData.ts,逐字照搬)。
// 算法整体对应反编译 bundle(chatcut-reverse/resources/bundles/decompiled/editor/entry.js)
// 的贪心断行路径:$b 收集断点候选 → 预算(词数/字符)触顶 → uHe 选分数最高候选回退断行
// → FHe 后处理(行首助词回拉)。各函数注释标注 bundle 定位正则。
//
// 与源站的两点有据偏差(均为任务规格指定):
// 1. 源站是 px 宽 + 原始字符数双预算(S1e/Gu 因子表,bundle 定位 /const Gu = \{/,
//    CJK:LATIN = 1:0.55);本引擎折成单一「字符单位」预算:CJK 字符=2、其余=1。
// 2. 源站对非 CJK + wordsPerPage 走纯计数分页(bundle 定位 /function Y9e\(/、/function Z9e\(/,
//    不打分);本引擎按任务规格对词数预算触顶同样走打分回退。
// 未搬:纯英文单行 DP 优化器(Q9e/dr 代价表)与行均衡后处理(AHe/NHe/vHe)——预设逐个启用时再补。
import {
...
export interface SegmentOpts {
  ...
  /** 每页词数预算(标点词不计)。CJK 主导文本且给了 maxCharsPerLine 时忽略(源站 M1e 语义)。 */
  wordsPerPage: number;
}
...
const CJK_START = /[㐀-鿿぀-ヿ가-힯]/u; // bundle 定位 /function aHe\(/
const PUNCT_ONLY = /^[\p{P}]+$/u; // bundle 定位 /bUe\s*=/
const CJK_PUNCT_CHARS = /[，。！？；：、“”‘’（）【】《》「」『』〈〉〔〕｛｝〖〗…—～·]|[｡､]/;
const LATIN_PUNCT_CHARS = /[.,!?;:'"()[\]{}/\\@#$%^&*\-+=<>|~`]/;

type WordScript = 'punctuation' | 'number' | 'cjk' | 'latin' | 'mixed';

/** 字符分类(bundle 定位 /function pI\(/):0=CJK 1=小写拉丁 2=大写拉丁 3=数字 4=标点 5=空格 6=其他。 */
function charClass(ch: string): number {
  ...
  return 6; // ponytail: 源站对代理对还有细分,此处一律按其他计
}

function hasCjkChar(text: string): boolean { // bundle 定位 /function Zc\(/
  for (const ch of text) if (charClass(ch) === 0) return true;
  return false;
}

function isPunctOnly(text: string): boolean { // bundle 定位 /function Gi\(/
  const t = text.trim();
  return t.length > 0 && PUNCT_ONLY.test(t);
}

/** 词的文种分类(bundle 定位 /function g1e\(/)。 */
...
/** 两词之间的接缝文本(bundle 定位 /function lu\(/ 与 /function w1e\(/)。 */
...
/** CJK 占比 ≥0.3 判为 CJK 主导(bundle 定位 /function fI\(/)。 */
...
/** 英文断点打分器(bundle 定位 /function H9e\(/):遍历相邻词对,基础 20 分,
 * z9e 命中改分、U9e 首个命中扣罚、句末 /[.!?]$/ +30、j9e 孤词且剩词 ≤2 时 −40。 */
...
/** CJK 标点断点优先级(bundle 定位 /function oHe\(/):句末 100 / 逗顿 80 / 引号收 70。 */
...
/** 语气词后断(bundle 定位 /function aHe\(/):左词末字 ∈ W9e 且右词以 CJK 开头。 */
...
/** mA() 孤词避断(bundle 定位 /function mA\(/):左词末字或右词首字 ∈ Q9 → orphanRisk。 */
...
/** CJK 疑问句式断点(bundle 定位 /function lHe\(/、/function eH\(/):
...
/** 「idx−1 与 idx 之间不可拆」(bundle 定位 /function wu\(/ + /function hI\(/ + /function b1e\(/):
 * 无接缝且 Intl.Segmenter 判定落在同一 CJK 词内(或命中 y1e 词缀)。整段文本只分词一次。 */
...
/** 英文相邻词对断点(bundle 定位 /function cHe\(/):±2 词窗口跑 H9e,取离断点最近
...
/** 选最优断点(bundle 定位 /function uHe\(/):有效分 = priority − 孤词降权 30,
 * 同分取靠后;不可拆处剔除。 */
...
/** FHe 后处理(bundle 定位 /function FHe\(/ + /function RHe\(/):某页首词以 G9e
...
export function segmentWords(words: SegmentWord[], opts: SegmentOpts): number[] {
  ...
  // M1e 语义(bundle 定位 /function M1e\(\{/):CJK 主导且 wordsPerPage>1 时词数预算置空
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (over && !punct && i > pageStart) {
      ...
      if (best) {
        ...
        units = nextUnits; // 粘着词无法拆,容忍超预算(源站 J9e 同款)
        count = nextCount;
      }
    } else {
      units = nextUnits;
