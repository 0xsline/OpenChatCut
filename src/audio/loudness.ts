// 响度归一分析核心。源站无据(自定): `复刻规格-Agent工具与后端.md` §8 只有
// `isolate_voice`(DeepFilterNet3 语音增强),没有独立的响度归一工具——"duck depth
// 自动从 loudness 初始化"只是 auto-duck 的副作用,不是可调用工具。按 CLAUDE.md
// 规则5"源站无据,自定"补一个命名忠实(submit_*/edit_*/manage_* 之外,动词_名词,
// 同 isolate_voice 风格)的新分析+工具。
//
