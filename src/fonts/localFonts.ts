/**
 * 源站内置中文展示字体(bundled CJK display fonts)—— 运行时。
 *
 * 出处(逆向实锤):chatcut-reverse/resources/bundles/decompiled/editor/entry.js
 * `const JI = [...]`(12 项,已用 node 求值原样核对)。源站运行时消费方式一致:
 * `new FontFace(family, url(<path>), { display:'swap', weight })` → document.fonts。
 * 二进制已从源站公开 CDN(app.chatcut.io/fonts/)取回,落在
 * public/fonts/<slug>/<file>.woff2(13 文件,共 ~38MB),路径与本表一一对应。
 *
 * 加载策略:registerLocalFonts() 只注册 FontFace(unloaded)——浏览器与
 * @font-face 同语义,排版真正用到该 family 时才拉 woff2,38MB 不会在启动时
 * 全量下载;ensureLocalFont(family) 是显式 await 版(load 完成才 resolve)。
 * headless 渲染:render bundle overlay 了 public/,同源 /fonts 路径同样可载,
 * 无需特判(本注释即说明)。
 */
...
export interface LocalCjkFont {
  ...
  family: string;
  /** 源站 importName,同时作为可搜别名。 */
  importName: string;
  /** 源站中文别名。 */
  aliasZh: string[];
  /** weight → 同源 URL(public/fonts 静态文件)。 */
...
export const LOCAL_CJK_FONTS: readonly LocalCjkFont[] = [
  ...
  // 抖音美好体 — 抖音美好体授权(字节跳动,免费商用);源站同一 Bold 文件充 400+700
  ...
  // OPPO Sans — OPPO Sans 免费商用授权(OPPO);源站同一 Regular 充 300–700 五档
  { family: 'OPPO Sans', importName: 'OPPOSans', aliasZh: [],
    files: { 300: '/fonts/oppo-sans/OPPOSans-Regular.woff2',
             400: '/fonts/oppo-sans/OPPOSans-Regular.woff2',
             500: '/fonts/oppo-sans/OPPOSans-Regular.woff2',
             600: '/fonts/oppo-sans/OPPOSans-Regular.woff2',
             700: '/fonts/oppo-sans/OPPOSans-Regular.woff2' } },
  // 新青年体 — 免费商用(以原发布方授权页为准);同一 W8 文件充 400+700
  { family: 'Xinqingnian Ti', importName: 'XinqingnianTi', aliasZh: ['新青年体', '新青年'],
    files: { 400: '/fonts/xinqingnian-ti/XinqingnianTi-W8.woff2',
             700: '/fonts/xinqingnian-ti/XinqingnianTi-W8.woff2' } },
];

/** family / importName / 中文别名 → 条目(归一化匹配,源站 WO() 同语义)。 */
