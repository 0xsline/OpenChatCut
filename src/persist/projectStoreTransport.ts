...
function clearStoredLaunchToken(expected: string): void {
  // Undefined (not null) so a later credential is not masked by a cached miss.
  if (launchToken === expected) launchToken = undefined;
  try {
  ...
  try {
    if (localStorage.getItem(LAUNCH_STORAGE_KEY) === expected) {
      localStorage.removeItem(LAUNCH_STORAGE_KEY);
    }
  } catch {
    // Best-effort; the persistent copy may linger in privacy-restricted environments.
  }
}
...
function consumeLaunchToken(): string | null {
  // Capture and scrub a freshly-arrived URL fragment on EVERY call: a new
  // launch credential can be written by the launcher while an older exchange
  // is still pending (main's captureLaunchFragment semantics).
  if (typeof location !== 'undefined' && typeof history !== 'undefined') {
    try {
      const params = new URLSearchParams(location.hash.startsWith('#') ? location.hash.slice(1) : '');
      const candidate = params.get(TOKEN_FRAGMENT_KEY)?.trim() ?? '';
      if (candidate.length >= 32) rememberLaunchToken(candidate);
      if (params.has(TOKEN_FRAGMENT_KEY)) removeLaunchFragment(params);
    } catch {
      // Non-browser environments leave the fragment for the next consume.
    }
  }
...
function clearStoredSession(expected?: string): void {
  ...
  // Undefined (not null) so a later session is not masked by a cached miss.
