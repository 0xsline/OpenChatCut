// 工程模板库(source manage_template):模板 = 一组 MG + 设计风格的打包。
// 一个模板就是一份打包好的 ProjectDoc(它的时间线里的 MG 片段 + designStyle +
// 携带的媒体资产)。跨工程共享(像"我的设计风格"收藏一样是全局库,不按工程分),
// 存在 projectStore 的同一个 kv 库,但这里自己开同一个库(idb helper 是模块私有的),
// 不改 projectStore.ts。读取时一律用 migrateProjectDoc 校验(持久化数据不可信)。
