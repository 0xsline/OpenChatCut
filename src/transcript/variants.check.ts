// Runnable source-contract check: `npx tsx src/transcript/variants.check.ts`.
// 聚焦多语言转写变体 + 护城河③(词↔帧双向一致):变体只承载文本,时间戳恒取源。
...
// 护城河③:每个词的 start/end/speaker 恒等于源词
...
// ── 3) 护城河③ 硬证:套用变体后 retime 的时间轴,与用源词 retime 完全一致 ──
...
assert.strictEqual(item.durationInFrames, 24, 'clip duration untouched (护城河③)');
