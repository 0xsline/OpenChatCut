...
async function main(): Promise<void> {
  ...
  const kfs = focalFramesFromGeometry(asset, 900, FPS, { intervalFrames: 15 });
  ...
  const shifted = focalFramesFromGeometry(asset, 300, FPS, { srcInFrame: 300, intervalFrames: 15 });
  ...
  const slowed = focalFramesFromGeometry(asset, 660, FPS, { playbackRate: 0.5, intervalFrames: 15 });
  ...
  const faceKfs = focalFramesFromGeometry(faceOnly, 150, FPS);
  ...
  const endKfs = focalFramesFromGeometry(asset, 100, FPS, { intervalFrames: 30 });
