// 自动 reframe 检测核心。
// 源站无据,自定(CLAUDE.md 对标源站 §5):ChatCut 源站只有 reframe 曲线的“写入/渲染”
// 基础设施(builtin:zoom + reserved __chatcutReframeCurve = ReframeCurveV1),并没有
// “采样视频 → 检测主体 → 生成 reframe 关键帧”的自动检测工具。本文件的启发式检测
// (逐格对比度/方差能量 → 显著区域质心)是本复刻自定的轻量 MVP,零重依赖
...
const DEFAULT_MAX_SAMPLES = 40; // seek 采样上限,防止长片卡死
...
export async function detectFocalPoints(source: HTMLVideoElement | FrameSampler, opts: DetectOptions): Promise<DetectedKeyframe[]> {
  ...
  for (const frame of frames) {
    ...
    if (!cap) continue;
    const focal = cap.faceFocal ?? focalFromEnergyGrid(emphasize(energyGridFromImageData(cap.img, cols, rows), sensitivity));
    out.push({ frame, focalPointX: clamp01(focal.x), focalPointY: clamp01(focal.y), magnification });
  }
  return out;
}
