// apply_script's diff engine (source mcp-tools-schema.md:980-1028): diff the
// edited timeline.md against the live canonical state and map it back to
// DETERMINISTIC editor commands. Row identity = [sN]/[cN] ids + ### source
// mapping; kept text is content-matched against the source segment (tolerant
...
// (plan first, dispatch after = atomic like the source).
...
export interface ApplyResult {
  ok: true;
  /** rows/clips that disappeared vs the canonical state (source `Removed:` audit) */
  removed: string[];
  changes: string[];
}
