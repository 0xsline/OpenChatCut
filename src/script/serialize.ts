// timeline.md serialization (source read_script, mcp-tools-schema.md:919-968).
    ...
    for (const it of items) {
      ...
      if (it.transcript?.length) {
        ...
        toSegments(it.transcript).forEach((seg, i) => {
          ...
          if (!kept.length) return; // fully-deleted segment: absent (like source re-materialization)
...
/** state → timeline.md (source read_script). Also returns the model + stamp. */
