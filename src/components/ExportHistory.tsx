...
// Export history (源站无据, 自定). Self-contained: renders its own topbar trigger
// button AND the popover. GLOBAL history (single-user clone) — loads straight from
...
export function ExportHistory() {
  ...
  return (
    <>
      <button title="导出历史" onClick={() => setOpen(true)}
        ...
        <div onClick={() => setOpen(false)} style={backdrop}>
          <div onClick={(e) => e.stopPropagation()} style={card}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '13px 16px', borderBottom: `1px solid ${theme.border}` }}>
              ...
              <span style={{ flex: 1, fontSize: 14, fontWeight: 600 }}>导出历史</span>
              <button onClick={() => setOpen(false)} title="关闭" style={iconBtn}><Icon name="x" size={15} /></button>
            </div>
            ...
            <div style={{ padding: 12, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6, minHeight: 120 }}>
              {loading ? (
                ...
                <div style={emptyState}>还没有导出记录</div>
              ) : (
                records.map((r) => (
...
const ghostBtn: React.CSSProperties = {
  ...
  borderRadius: 7, padding: '5px 12px', fontSize: 12.5, whiteSpace: 'nowrap',
};
