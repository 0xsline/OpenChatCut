...
export type FieldKind = 'secret' | 'text' | 'select';
...
const LLM_MODEL_FIELD: SettingsField = {
  name: 'LLM_MODEL', label: '模型', kind: 'text', defaultLabel: 'grok-4.5-latest',
  note: '也可填 MiniMax M 系模型（Base URL 用 https://api.minimaxi.com/anthropic）。',
  options: ['grok-4.5-latest', 'claude-sonnet-4-5', 'claude-opus-4-5', 'MiniMax-M2.5', 'MiniMax-M3']
...
export const SETTINGS_CATEGORIES: readonly SettingsCategory[] = [
  {
    ...
    groups: [
      { key: 'llm', title: 'Agent 大脑',
        ...
        vendors: [
          { key: 'llm/anthropic', vendor: 'anthropic', title: 'Anthropic 兼容中转', fields: [
      ...
      { key: 'video', title: '生视频', hint: 'submit_video · 文 / 图生视频，任一厂商即可。',
        ...
        vendors: [
          ...
          minimaxPage('video', modelSelect('MINIMAX_VIDEO_MODEL', '视频模型', 'MiniMax-Hailuo-02',
            ['MiniMax-Hailuo-02', 'MiniMax-Hailuo-2.3', 'MiniMax-Hailuo-2.3-Fast']), 'MiniMax 海螺', 'hailuo'),
        ] },
      { key: 'music', title: '生音乐', hint: 'submit_music · 文字生成配乐，任一厂商即可。',
        ...
        vendors: [
          ...
          minimaxPage('music', modelSelect('MINIMAX_MUSIC_MODEL', '音乐模型', 'music-2.6',
            ['music-2.6', 'music-2.6-free', 'music-cover'])),
        ] },
    ],
  },
  {
  ...
  },
  {
    ...
    groups: [
      { key: 'sandbox', title: '沙箱执行', hint: 'run_code · ffmpeg / node / python 媒体探测与处理。',
        vendors: [
          { key: 'sandbox/e2b', vendor: 'e2b', title: 'E2B',
            fields: [secret('E2B_API_KEY', 'API Key'), text('E2B_TEMPLATE', '模板 ID（可选）')] },
        ] },
...
export function isModelField(field: SettingsField): boolean {
  return field.kind === 'select' || field.defaultLabel !== undefined;
}
...
export function selectOptions(field: SettingsField): readonly SelectOption[] {
  ...
  return [{ value: '', label: `默认（${field.defaultLabel}）` }, ...base];
}
...
export function selectOptionLabel(
  ...
  return ok ? opt.label : `${opt.label}（未配置）`;
}
...
export function fieldPlaceholder(field: SettingsField, configured: boolean, stagedClear: boolean): string {
  if (isModelField(field)) {
    return stagedClear ? '恢复默认 · 保存后生效' : `默认 ${field.defaultLabel ?? ''}`.trimEnd();
  }
