...
// Interactive follow-up form card (source ask_followup_questions / <widget>).
// Redesigned to match proposal card: warm dark panel, custom radio/check chips.
export function WidgetCard({ fields, onSubmit }: WidgetCardProps) {
  ...
  return (
    <div className={`cc-widget${submitted ? ' submitted' : ''}`}>
      <div className="cc-widget-glow" aria-hidden />
        ...
        {fields.map((f, fi) => (
          <section key={f.id} className={`cc-widget-field${fi === 0 ? ' first' : ''}`}>
            <header className="cc-widget-field-head">
              ...
              {f.required ? <span className="cc-widget-req">必选</span> : <span className="cc-widget-opt">可选</span>}
            </header>
              ...
              <div className="cc-widget-options" role="radiogroup" aria-label={f.label}>
                ...
                {f.allowOther && (
                  <label className={`cc-widget-option cc-widget-other${otherFields[f.id] ? ' on' : ''}${submitted ? ' disabled' : ''}`}>
                    ...
                    />
                    <span className="cc-widget-radio" aria-hidden />
                    <span className="cc-widget-option-text">其他…</span>
                    {otherFields[f.id] && (
                      <input
                        type="text"
      ...
      <footer className="cc-widget-foot">
        {submitted ? (
          ...
          <span className="cc-widget-hint">选择后提交，Agent 会按你的选择继续</span>
        )}
        <button
          type="button"
