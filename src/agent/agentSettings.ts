...
/** MG 生成质量三档 (source: localStorage `chatcut-agent-motion-graphic-tier`). */
...
export interface AgentSettings {
   ...
   */
  skillGuard: boolean;
  /** 思考模式 (source: localStorage `chatcut-thinking-enabled-v3`;开 → 请求带 thinking:'adaptive' + effort:'medium')。 */
  thinkingEnabled: boolean;
  /** MG 质量档 (source: localStorage `chatcut-agent-motion-graphic-tier`,默认 balance),经 <agent_settings> 注入。 */
  mgTier: MgTier;
  /** 计划模式 (source: Agent Settings planMode 开关):先出编号计划,用户确认后再动手。 */
  planMode: boolean;
}
...
/**
 * 每请求注入的 <agent_settings> 段。源站 (Hy@24231) 把
 * `<agent_settings>motion_graphic_tier=${tier} … pass --tier ${tier}</agent_settings>`
 * 拼进每条用户消息;本仓等价物 = 追加到 system 组装尾部 (runtime.runAgent)。
 */
export function agentSettingsPrompt(s: Pick<AgentSettings, 'mgTier' | 'planMode'>): string {
  const lines = [
    `motion_graphic_tier=${s.mgTier}`,
    `生成 MG 时按档位取舍:speed=最快出活 / balance=均衡 / quality=打磨动效细节;调用 MG 生成时等效于 pass --tier ${s.mgTier}。`,
  ];
  if (s.planMode) {
    // 源站 planMode 内部提示词无据,自定
...
// 块(source agent-chat 20100-20150 两者都折成 thinking 块)。跨 chunk 状态机:
