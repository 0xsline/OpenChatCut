// Selection mode + rich @ references (source agent-chat bundle 29906–30080).
// The source lets the user "point at" things while composing: timeline clips
...
// mirroring the source contenteditable serializer (XM). Panels report picks via
// a window event (source: window "chatcut:items-clicked|time-marked|
// canvas-region-marked|transcript-selected"); a module store carries the
// selection-mode flag (source isSelectionMode) so panels deep inside other
...
// ── metadata per reference type (source field names kept verbatim) ──────────
...
export interface TimerangeMetadata {
  ...
  trackAlias?: string;
  timelineFrameStart: number;
  /** exclusive end frame (source timelineFrameEndExclusive) */
...
export interface CanvasRegionMetadata {
  fps: number;
  timelineId?: string;
  /** region in COMPOSITION coordinates (source: regionX/Y/Width/Height) */
  regionX: number;
  regionY: number;
  regionWidth: number;
  regionHeight: number;
...
export interface TranscriptSelectionMetadata {
  ...
  /** EDITED-timeline frames via keptSegments math (护城河③, undefined if all picked words are cut) */
  timelineFrameStart?: number;
  timelineFrameEnd?: number;
  speakerName?: string;
}
...
/** source reference `category` per type ('time' | 'region' | 'transcript' | 'item'). */
...
/** Prompt token for a mention (source XM serializer): time → @t[…], region →
...
/** Overlap test with touching edges counting as intersection (source P0). */
export function rectsIntersect(a: RegionRect, b: RegionRect): boolean {
  return a.x <= b.x + b.width && a.x + a.width >= b.x
    && a.y <= b.y + b.height && a.y + a.height >= b.y;
}

/** Visual clips visible at `frame` whose rect intersects the region (source jh:
...
// ── transcript selection → reference (词↔帧走 keptSegments 数学, 护城河③) ────
...
// useSelectionRefMode() and report picks with emitSelectionRef(). This mirrors
// the source's isSelectionMode + window "chatcut:*" CustomEvents and avoids
...
/** Reactive selection-mode flag for panels (source isSelectionMode). */
