// Runnable source-contract check: `npx tsx src/agent/highlight-tool.check.ts`.
// 覆盖智能切片(长转短)的确定性内核:N 段高光→N 条竖屏序列、裁到高光帧区间、
// 护城河③(词↔帧一致,切点落在词边界且不改任何词的文本/时间),以及对不可信 LLM
