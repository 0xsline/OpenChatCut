...
// (source: track_progress reads the job table; transcription runs client-side so the
...
// placed afterwards inherit the transcript (护城河③).
...
export async function execTranscriptionProgress(args: Args, ctx: AgentContext): Promise<unknown> {
  ...
  // Persist terminal ASR results onto the pool asset (source: asset 标"转写完成/失败").
