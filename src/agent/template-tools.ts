...
// manage_template — 工程模板(source 复刻规格 §9)。模板 = 一组 MG + 设计风格的打包。
// 源站 action: get / list_assets / apply(参数 templateId / placement / omitAssetIds)。
// 语义:apply 前先 list_assets 决定「套用现成 vs 重新生成」。
// save 为自定 action(源站未命名保存工序):把当前工程打包存为模板。
// apply 经 ctx.commands.applyDoc 单步原子提交(护城河①:一次可撤销的时间线整体变更)。
...
export const TEMPLATE_TOOL_SCHEMAS: Anthropic.Tool[] = [{
  ...
  description: [
    ...
    'save(带 name)=把当前工程打包保存为模板(同名覆盖)。源站未命名此保存工序,自定。',
  ].join(' '),
  input_schema: {
    type: 'object',
    properties: {
...
export async function execTemplateTool(name: string, args: Args, ctx: AgentContext): Promise<unknown> {
  ...
  switch (action) {
    ...
    case 'apply': {
      ...
      ctx.commands.applyDoc(clean); // 护城河①:一次原子、可撤销的时间线整体变更
