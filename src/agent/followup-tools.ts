...
// ask_followup_questions (source MCP tool, required: ['fields']): agent 主动向用户发一张
// 交互表单卡问关键信息。本仓 UI 早有 <widget> 解析(widget-parse.ts + WidgetCard),但源站
// 这是个正式 TOOL——之前只靠 systemPrompt 教模型吐 <widget> 文本、agent 无法"调用"它
// (旧 TODO 误记为 ✅)。这里补工具:exec 把 fields 序列化成同款 <widget> 文本,经 runtime 的
// __followup 特判渲染成卡片并暂停 loop 等用户作答(答案由现成 onWidgetSubmit 回填为下条用户
// 消息)。本仓 widget 支持 single/multi;源站还有 text 自由输入,本仓无自由框→降级为 prompt
