...
// transcript-backed notes(源):create 传 transcriptSegments("3"/"3-5"/逗号列表,
// 与 read_script 的 [sN] 同一套编号)即可省 fromFrame/note——帧位由词级时间戳换算
// (makeWordFrameMapper,与播放层同源),note 正文拷贝段文本,可挂 notePrefix 标签;
...
export const MARKERS_TOOL_SCHEMAS: Anthropic.Tool[] = [
  {
    name: 'manage_markers',
    description: [
      '时间线批注/TODO 锚点(source manage_markers,契约 marker-note-v2)。marker = 点(durationFrames 0)或段(>0),',
 ...
 *  护城河③);note = 所选段的 keptText(即 read_script 显示的文本)。 */
...
function createOpts(o: Args, state: TimelineState): { fromFrame: number; opts: Parameters<AgentContext['commands']['addMarker']>[1] } | { error: string } {
  ...
  if (spec) {
    // 源语义:显式 fromFrame/note 优先;缺省从所选段派生(note 可挂 notePrefix 标签)
