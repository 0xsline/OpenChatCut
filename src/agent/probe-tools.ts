// probe_media — accurate ffprobe metadata for an import, run in our e2b sandbox via the
// existing /e2b/run proxy. This is the faithful slice of the source asset-import helper
// (upload-media.mjs): its probe step runs `ffprobe -show_streams -show_format` and reads
// codec_type to learn hasAudioTrack / fps / duration / dimensions. The source has NO
// standalone probe tool (probing lives inside the 2348-line helper), so this tool name is
// self-defined (源站无独立工具, 探测在 upload-media.mjs 内, 自定). The agent probes BEFORE
// finalize_uploaded_asset so it can pass an accurate hasAudioTrack — silent / no-audio
// media then skips 上传即转写 ASR (see A1), and fps/duration are exact.
