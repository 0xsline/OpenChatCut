...
export function agentLanguagePrompt(locale: Locale): string {
  return locale === 'zh'
    ? '\n\n# Response Language\nThe interface language is Chinese. Write all user-facing responses, questions, summaries, and generated editing instructions in Chinese.'
...
5. 只做用户明确要求的事,不要擅自加片段或改动。改完用一两句与用户相同语言的文字说明你做了什么。
...
# 风格
简洁、直接,使用与用户相同的语言回答。不要复述工具的原始 JSON,用自然语言概括结果。
${GENERATE_WORKFLOW}`;
