...
// edit_asset (source 域D §6): 改/删「库资产」(媒体池里的 asset,非时间线片段)。
// - update: 改名 / 改 props / 对 code 类资产(MG)换源码 — 换 code 必须先过 MG 沙箱
//   (护城河②),不过不落。MediaAsset 无存储缩略图(MG 预览按 code 现渲),无需失效。
// - delete: 从池里移除。confirmImpact:若有片段引用它(MG 按 templateId、媒体按 src),
//   先报计数、要 confirm:true 才删(片段自身已拷贝 code/持有 src,删池条目不破坏它们)。
// 改名也在 manage_media_pool.rename_asset;这里为忠实 edit_asset 一并支持。
...
function update(asset: MediaAsset, args: Args, ctx: AgentContext): unknown {
  ...
  if (code) {
    ...
    try {
      compileTemplate(code); // 护城河②:静态黑名单 + 受限作用域编译,双门过了才落
    } catch (e) {
