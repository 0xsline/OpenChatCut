...
// normalize_loudness —— 响度归一(目标默认 -14 LUFS,流媒体平台标准)。源站无据
// (自定):`复刻规格-Agent工具与后端.md` §8 只有 `isolate_voice`(DeepFilterNet3
// 语音增强),没有独立的响度归一工具——"duck depth 自动从 loudness 初始化"只是
// auto-duck 的副作用,不是可调用工具。按 CLAUDE.md 规则5"源站无据,自定"补一个,
// 命名忠实(动词_名词,同 isolate_voice/edit_captions 风格)。
//
