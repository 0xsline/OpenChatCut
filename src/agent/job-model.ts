// 统一异步 Job 模型(计划 A3)。
// ---------------------------------------------------------------------------
// 源站后端是一张 job 表(generation_job/analysis_job),字段 status/result 一套状态机,
// 通过两个"同构"轮询工具暴露给 agent:track_progress(generation/transcription/upload/
// visual-analysis)与 track_export(render)。见 chatcut-reverse 复刻规格 G3 + §7/§10。
//
// 克隆把这套 job 拆成三个"家族",落在不同的执行地(locality),各自的 wire 状态词汇:
...
// 这些 wire 字符串**对各自家族是忠实的**,保持原样不改:
//   · 同步 submit_export(generate-tools.ts,grok 冻结)返 status:'completed' → 导出族终态词就是 completed;
//   · 生成族终态词是 succeeded;两个家族在源站本就是 track_progress vs track_export 两条线,词汇可不同。
//
...
//
// 备注:源站 track_progress 还有 upload | visual-analysis 两个 target,克隆尚未实现这两类
// job,故 JobKind 暂不含(实现时再加)。
...
/** 已实现的 job 家族(源站 track_progress target 的子集 + 渲染族)。 */
export type JobKind = 'generation' | 'transcription' | 'export';
...
/** 每个家族的工具面向 job 报告都满足的共享骨架。job 句柄字段名按源站自己的
