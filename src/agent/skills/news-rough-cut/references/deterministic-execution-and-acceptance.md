...
1. **环境与素材检查（前置，必做）**
   ...
   - 从首次 `read_project` 的时间线与媒体池中记录全部初始 `video` / `audio` 的 `sourceAssetId` 与 `src`，形成允许来源基线。
...
5. **终检验收（强制性，见下）**
   - 再次 `read_project`，逐项比对所有最终 `video` / `audio` 的 `sourceAssetId` / `src` 与允许来源基线；同一来源的 trim、split、move 合法，任何新来源（包括新增视频中携带的声音）均不合法。
...
| 无外部声音 | 所有最终 `video` / `audio` 的 `sourceAssetId` / `src` 均属于编辑前允许来源基线；允许原来源的裁剪/切分/重排 | 移除新来源片段并重新验收 |
...
- 验收里的"无新增音频"必须以首次与最终两次 `read_project` 的来源基线比对为准，不能按 audio 轨片段数量判断。
