...
import { keptSegments, type EditOpts } from '../transcript/edit';
import { msToFrame, type TranscriptWord } from '../transcript/types';

// find_transcript — 源站参数面:query(必填) + asset / track / fuzzy /
// includeWordTimestamps / limit。时间坐标查询器:定位一句话何时被说出,把
// B-roll/MG/marker/overlay 锚到那一刻。timeline 模式尊重剪辑(删词不再命中);
// asset 模式查资产 RAW 转写(库查询,无视剪辑)。词→帧换算与播放层同源
// (keptSegments,护城河③ 词↔帧一致)。markers 的 transcriptSegments 也复用
...
export function makeWordFrameMapper(item: TimelineItem, fps: number): (gi: number) => { fromFrame: number; toFrame: number } | null {
  ...
  const opts: EditOpts = { maxGapFrames: item.silenceFrames, gapCapsMs: item.gapCapsMs, playOrder: item.transcriptPlayOrder };
