...
// auto_reframe — 源站无据,自定(CLAUDE.md 对标源站 §5)。
// ChatCut 源站的 reframe 只有“写入/渲染”基础设施(builtin:zoom + reserved
// __chatcutReframeCurve = ReframeCurveV1),没有“采样视频→检测主体→自动生成关键帧”
...
export async function execReframeTool(name: string, args: Args, ctx: AgentContext): Promise<unknown> {
  ...
  try {
    ...
    return { ok: true, itemId: item.id, keyframes: keyframes.length, magnification: mag, dstAspect: Number(dstAspect.toFixed(4)) };
  } catch (err: unknown) {
