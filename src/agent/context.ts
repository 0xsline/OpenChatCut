 ...
 * the five selection-mode reference types (source: item / timepoint / timerange
 ...
 * refs re-resolve against the LIVE state at send time (source C0 refresh);
 * transcript refs pick up the pool assetId; a timeline mismatch is surfaced as
 * a warning instead of silently retargeting (source Cc). */
