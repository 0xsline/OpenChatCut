...
// source read_project — single overview of project state (timeline + assets).
// Local clone aggregates existing store/doc fields; no separate backend.
...
export const READ_PROJECT_TOOL_SCHEMAS: Anthropic.Tool[] = [
  {
    name: 'read_project',
    description: [
      'View the project — tracks, timeline items, markers, media-pool folders, and assets (source read_project).',
    ...
    input_schema: {
      ...
      properties: {
        ...
        projectId: { type: 'string', description: 'Ignored in local clone (single open project).' },
      },
    },
  },
];
