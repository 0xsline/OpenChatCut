// Runnable source-contract check: `npx tsx src/agent/template-tools.check.ts`.
// Covers manage_template (源站 get / list_assets / apply,+ 自定 save):
//   save 当前工程打包成模板 → get(list) 列出、get(detail) 回带其 doc(MG + 设计风格);
//   list_assets 返回携带资产;apply 经 applyDoc 单步原子提交(护城河①),结果含模板的
//   MG 片段与设计风格;omitAssetIds 跳过资产及引用它的片段;placement=replace 换掉现有内容;
//   未知 templateId → 干净报错且不动工程。
// 模板库经 templateStore 的 IndexedDB helper 持久化,故装同款内存 IndexedDB shim
...
// 护城河①: exactly ONE atomic, undoable change (a single tl.setDoc)
