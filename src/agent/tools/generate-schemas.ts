...
export const GENERATE_TOOL_SCHEMAS: AgentToolSchema[] = [
  ...
  {
    name: 'submit_music',
    description: 'Submit a Mureka, MiniMax, or Sonilo music job. Mureka supports instrumental, lyrics song, prompt song, soundtrack, and track/stem modes with up to 3 results. MiniMax supports text-to-music and cover. Sonilo v2m generates music from a project video asset (the rendered cut, up to 6 minutes) — video-conditioned, with a single optional style prompt. Results are saved to the media pool.',
    input_schema: {
      type: 'object',
      properties: {
        ...
        provider: { type: 'string', enum: ['mureka', 'minimax', 'sonilo'], description: 'Defaults to mureka.' },
        mode: { type: 'string', enum: ['instrumental', 'song', 'prompt-song', 'soundtrack', 'track', 't2m', 'cover', 'v2m'], description: 'Mureka: instrumental/song/prompt-song/soundtrack/track. MiniMax: t2m/cover. Sonilo: v2m (video-to-music).' },
