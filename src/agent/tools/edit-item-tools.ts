...
// envelope/shape goes RAW propertyOverrides(cleanOverrides only leaves numbers and ≤4 digit arrays,
// Will swallow 2..120 bit envelope and string shape).
...
function validateTransitionAdd(ctx: AgentContext, entry: Record<string, unknown>): OpResult {
  const assetId = String(entry.assetId ?? '');
  // custom:tr-* (submit_shader product) and plugin:<pack>/<item> (installed plugin transition) are both there
...
function validateEffectUpdate(ctx: AgentContext, entry: Record<string, unknown>): OpResult {
  ...
  // Live lessons: Models often send effect update to "clips that don't have effects yet" → Feed back the existing effects list/addition guide
  if (!it || index < 0) {
    return {
