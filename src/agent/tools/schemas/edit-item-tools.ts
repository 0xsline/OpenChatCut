      ...
      properties: {
        ...
        updates: {
          ...
          description:
            ...
            + 'filters: {brightness?,contrast?,saturate? 0..2 (1=normal), blur? 0..30 px} on visual clips. transform: {scale? 0.05..16, x?/y? % of canvas, rotation? deg, opacity? 0..1, borderRadius?}. backgroundFill:true fills unused canvas with a blurred copy; backgroundFillPreset selects soft|medium|strong|maximum and enables the fill when used alone. Only video/image clips on the bottom video track (V1). speed/playbackRate: 0.1..8 on video/audio/gif (retimes duration). '
