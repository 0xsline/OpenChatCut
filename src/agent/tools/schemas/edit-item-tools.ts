    ...
    input_schema: {
      ...
      properties: {
        adds: {
          type: 'array',
          description:
            'effect: {type,targetItemId,assetId,propertyOverrides?}. transition: {type,assetId,incomingItemId,outgoingItemId?,durationInFrames?}. motion-graphic: {type,assetId:library:motion-graphic:*,track?,fromFrame|startFrame?}. audio SFX: {type:"audio",assetId:library:sound:*,fromFrame?}. POOL media B-roll (video/image/gif/svg/audio): {type,assetId,track|trackId?,fromFrame|startFrame?,durationInFrames?} — no name/props on pool adds (then update_item_props). Source-window add (video/audio/gif): {type,assetId,sourceStartSeconds?,sourceEndSeconds?,track?,fromFrame?} — pass search_media hit times straight through (seconds); the tool converts to frames, do NOT combine with durationInFrames/srcInFrame, and do not multiply by fps yourself. Authored text: {type:"text",text?,fontSize?,color?,fontWeight?,align?,name?,track|trackId?,fromFrame?,durationInFrames?} (no assetId). Authored solid: {type:"solid",color?,name?,track?,fromFrame?,durationInFrames?}. fromFrame omitted appends.',
          items: { type: 'object' },
        },
        updates: {
          type: 'array',
