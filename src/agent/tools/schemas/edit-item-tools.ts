...
export const EDIT_ITEM_TOOL_SCHEMAS: AgentToolSchema[] = [
  {
    ...
    description:
      ...
      + 'updates move/trim/retime by itemId|id. Passing assetId on a file-backed clip atomically replaces it from the media pool while preserving item id, track, timeline start, duration, and visual/audio edit state. '
      ...
      properties: {
        ...
        updates: {
          ...
          description:
            ...
            + 'filters: {brightness?,contrast?,saturate? 0..2 (1=normal), blur? 0..30 px} on visual clips. transform: {scale? 0.05..16, x?/y? % of canvas, rotation? deg, opacity? 0..1, borderRadius?, crop?: {left?,right?,top?,bottom?} composition pixels (Crop Left/Right 0..canvas width, Crop Top/Bottom 0..canvas height; cropped pixels are fully transparent; null clears crop)}. backgroundFill:true fills unused canvas with a blurred copy; backgroundFillStrength sets an exact integer percentage from 0 to 100 and enables the fill when used alone. Only video/image clips on the bottom video track (V1). speed/playbackRate: 0.1..8 on video/audio/gif (retimes duration). '
            + 'No CSS layout fields (left/right/top/bottom/width/height) — position clips via transform or keyframes x/y; clip edge crop belongs in transform.crop, not top-level left/right/top/bottom. layout INSIDE an MG belongs in its code/props. '
