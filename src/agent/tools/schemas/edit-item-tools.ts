      ...
      properties: {
        ...
        updates: {
          ...
          description:
            ...
            + 'Do NOT set assetId (use deletes+adds to place a different pool asset). keyframes: {x|y|scale|rotation|opacity|volume:[{frame,value,easing?}]} item-local frames; easing is stored on the left keyframe and controls the following interval. It accepts linear/easeIn/easeOut/easeInOut and CSS aliases ease-in/ease-out/ease-in-out. clearKeyframes:true clears all, or clearKeyframes:"opacity" for one prop. '
