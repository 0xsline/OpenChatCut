  ...
  return (
    <div className="cc-sound-browser">
      <label className="cc-sound-search" htmlFor="cc-sound-library-search">
        ...
        />
        {query ? (
          <button type="button" className="cc-sound-search-clear" onClick={() => setQuery('')} aria-label="清除">
            <Icon name="x" size={12} />
          </button>
        ) : null}
      </label>

      <div className="cc-sound-chips" role="tablist" aria-label="音效分组">
        <button
          type="button"
          role="tab"
      ...
      {list.length === 0 ? (
        <div className="cc-sound-empty">此分类下暂无音效{query ? `（与「${query}」不匹配）` : ''}</div>
      ) : (
        <div className="cc-sound-list" role="listbox" aria-label="音效列表">
          {list.map((s) => {
            ...
            return (
              ...
              >
                <button
                  ...
                  aria-label={isPlaying ? `暂停 ${s.name}` : `试听 ${s.name}`}
                >
