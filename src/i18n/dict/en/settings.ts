...
export default {
  ...
  'OpenAI · Responses / Chat': 'OpenAI · Responses / Chat',
  '请填写名称和 API URL': 'Enter a name and API URL',
  '名称无效': 'Invalid name',
  '该厂商已存在': 'Provider already exists',
  '保存到 .env.local': 'Save to .env.local',
  'ZCode 风格：名称 + Base URL + 协议 + 模型 + Key。保存后写入': 'ZCode-style: name + Base URL + protocol + model + key. Writes to',
  'ZCode 风格：名称 + Base URL + API format + 模型 + Key。保存后写入': 'ZCode-style: name + Base URL + API format + model + key. Writes to',
  '走 /messages（Grok / Claude 网关常用）': 'Uses /messages (common for Grok / Claude gateways)',
  '走 /chat/completions（多数兼容网关）': 'Uses /chat/completions (most compatible gateways)',
  '走 /responses（OpenAI Responses API）': 'Uses /responses (OpenAI Responses API)',
  '自定义厂商（ZCode 风格）。保存后写入 .env.local，密钥不进浏览器。': 'Custom provider (ZCode-style). Saves to .env.local; keys never enter the browser.',
  '对话与工具调用的核心，未配置无法对话。点 + 可添加自定义厂商（写入 .env.local）。': 'Core chat + tools. Unconfigured = no chat. Use + to add a custom provider (writes .env.local).',
  '关闭': 'Close',
  '收起': 'Collapse',
  '展开': 'Expand',
  '显示明文': 'Show values',
