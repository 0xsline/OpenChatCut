// FCPXML 序列化器（source submit_export format=xml, nleFormat fcp_xml/fcp_xml_resolve）。
 ...
 * 源站无据，此处是最简单能保证精确往返换算的写法，不追求 NTSC 1001/30000
 * 的行业惯例分母。
 */
...
export function timelineToFcpxml(state: TimelineState, opts: { title?: string } = {}): string {
  ...
  const spineChildren = sortedItems
    .map((item) => itemToSpineElement(item, fps, laneOf(item.track), assets))
    .join('\n        ');
