...
// contract mirrors 护城河③ (词↔帧一致) in spirit — for every rendered frame,
// pre-split and post-split sampling agree exactly (straddled bezier segments are
// de-Casteljau-subdivided, never approximated).
// 源站无据,自定:bundle 只确认「钢笔工具打关键帧 + bezier 缓动 + 五属性可 K 帧」,无算法细节。
import type { ItemKeyframes, Keyframe, KeyframeEasing, KeyframeProp } from './types';

/** clamp ranges per keyframable prop (x/y = % of canvas; agent + reducer boundary). */
export const KEYFRAME_RANGE: Record<KeyframeProp, readonly [number, number]> = {
  x: [-400, 400],
  y: [-400, 400],
  scale: [0, 10],
  rotation: [-3600, 3600],
