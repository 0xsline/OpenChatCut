...
Kdenlive 式 **裁左 / 裁右 / 裁上 / 裁下**。被裁掉的像素不会绘制，预览里保持全透明（棋盘格）。

**入口：** 选中视频或图片片段 → **片段属性** → **基础** → **变换**，在 **圆角** 下方。

| 控件 | 边 | 范围 |
|---|---|---|
| 裁左 / 裁右 | 左 / 右 | `0` … 画布宽度，单位 **px** |
| 裁上 / 裁下 | 上 / 下 | `0` … 画布高度，单位 **px** |

对边保留最小剩余宽度/高度，避免片段被裁穿。内部仍按 0–1 画布比例存储；检查器与 Agent 使用像素。

**Agent / MCP：** `edit_item` 的 `transform.crop: { left?, right?, top?, bottom? }` 为画布像素。`transform.crop: null` 清除裁切。不要用 CSS 的 `left` / `right` / `top` / `bottom` 表示片段四边。

<p align="center">
  <img src="assets/readme-pic/clip-crop-inspector.png" alt="片段裁切：预览四边 1–4 对应检查器裁左、裁右、裁上、裁下滑杆，数值为像素" />
</p>

<p align="center">
  <sub>预览上的 <b>(1)–(4)</b> 分别对应裁左、裁右、裁上、裁下。数值是画布像素，不是百分比。</sub>
</p>
...
| 视觉 | WebGL 特效、LUT、色度键、缩放、片段四边裁切、转场、自定义 shader |
