...
export function validateMusicRequest(input: MusicRequest): ValidMusicRequest {
  ...
  if (provider !== 'mureka' && provider !== 'minimax' && provider !== 'sonilo') throw new Error('provider must be mureka, minimax, or sonilo');
  const inferredMode: MusicMode = provider === 'mureka' ? 'instrumental'
    : provider === 'sonilo' ? 'v2m'
      : input.referenceAudioPath || input.coverFeatureId ? 'cover' : 't2m';
