...
// After configuring R2, upgrade to "write-through + return to source": the disk becomes cache, R2 is the real source; the src path remains unchanged.
//
// Large files (local first):
// - The default upper limit is 10GB (can be overridden with UPLOAD_MAX_BYTES); Content-Length exceeds the limit directly 413
...
export function uploadPlugin(): Plugin {
  return {
    ...
    configureServer(server) {
      // When the custom directory is enabled, the old assets in the default directory are copied in one direction (idempotent), and the single directory of the rendered symlink can see all.
      ...
      // GET /upload/list → Upload directory disk list (custom directory ∪ old default directory, remove duplicates by name).
      // The "Clean assets" panel uses it to compare with the full project reference set. Must be registered before /upload (connect prefix matches).
      ...
      // Used for cascading project deletion/cleaning up unowned assets. R2 Cloud objects are intentionally not moved (can still be retrieved from the source, local deletion = reversible).
        ...
        if (req.method === 'DELETE') {
          try {
            ...
            for (const dir of [uploadDir(), DEFAULT_UPLOAD_DIR]) {
              try { await unlink(join(dir, name)); removed += 1; } catch { /* ENOENT et al.*/ }
            }
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json');
